...
const QRCodeSection = props => {
  const { crowdFundingData, toggleQRCode, qrCodeOpen, isViewer,shareLink, openGlobalSnackbar } = props
  const downloadQR = () => {
    const canvas = document.getElementById('share-crowdfunding')
    const pngUrl = canvas
      .toDataURL('image/png')
      .replace('image/png', 'image/octet-stream')
    let downloadLink = document.createElement('a')
    downloadLink.href = pngUrl
    downloadLink.download = 'CrowdFundingQRCode.png'
        ...
        <div className="invoice-steps-card__options">
          <div className="invoice-step-Collapsible__header-content">
            ...
            <div className="d-flex cursor-pointer w-100" onClick={toggleQRCode}>
              <div className="py-heading--subtitle">Share your Give link</div>
            ...
            <div className="row align-items-start">
              ...
              <div className="col-3 d-flex align-items-end flex-column">
                {/* <QRCode
                  ...
                  includeMargin={true}
                /> */}
                <Button onClick={downloadQR} disabled={isViewer} outline color="primary px-4 mt-3">
                  Download code
                </Button>
              </div>
            </div>
